...
*/
console.log((fenomenler[6].profile = "Justin Bİeber"));
...
function indekseGoreFenomen(fenomenler, index) {
  const a = index + ". indekste bulunan fenomen: " + fenomenler[index].profile;
  return a;
}
...
function enFenomenler(fenomenler) {
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].followers > 100000000) {
      enCok.push(fenomenler[i].profile);
    }
  }
  return enCok;
}
...
function fenomenGonderimSayisi(fenomenler, fenomen) {
  let a;
  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].profile === fenomen) {
      a = fenomenler[i].posts;
    }
  }
  return a;
}
...
function platformaGoreCokGonderiYapanFenomen(fenomen, platform) {
  ...
  for (let i = 0; i < fenomen.length; i++) {
    if (fenomen[i].platform === platform) {
      if (fenomen[i].posts !== "NA" && fenomen[i].posts > max) {
        max = fenomen[i].posts;
        a = fenomen[i].profile;
      }
    }
